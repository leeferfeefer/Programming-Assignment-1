...
public class ServerTcp {
	...
	*/ 
	private static void createServerSocket(int port_number){
		...
		try {
			ServerSocket currentServerSocket = new ServerSocket(port_number);
			...
			while (true) {
				...
				//Socket data streams
			 	DataOutputStream outputStream = new DataOutputStream(currentClientSocket.getOutputStream());
			 	DataInputStream inputStream = new DataInputStream(currentClientSocket.getInputStream());
			 	...
			 	try{
					...
					try {
						if (isDebugMode) {
							...
							System.out.println("Destroying socket and data streams...");
						...
						if (isDebugMode) {
							System.out.println("Closing client socket unsuccessful...");
						}
						// e.printStackTrace();
					}
				}
	...
	*/ 
	private static void getMessage(DataInputStream inputStream, DataOutputStream outputStream) {
		...
		protocol.readClientMessage(inputStream, outputStream);
	}	
